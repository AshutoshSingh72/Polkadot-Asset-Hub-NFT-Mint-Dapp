...
api.tx.utility.batch([ await api.tx.nfts.mint(u32, u33, MultiAddress,  witnessData ), await api.tx.nfts.setMetadata(u32, u33, Bytes)]).signAndSend(SENDER, { signer: injector.signer }, async ({ status }) => {
    if (status.isInBlock) {
        toast.success(`Completed at block hash #${status.asInBlock.toString()}` , {
          position: "top-right",
          autoClose: 5000,
...
<hr className='hr'></hr>
<h2 className='count'>{u33} / 2,500 Minted</h2>
